...
As highlighted in Section 2.4, Meta-heuristic approaches, while powerful for solving complex problems, often require 
significant computational time. However, this time can be greatly reduced by 
optimizing the way these methods are applied. For example, instead of generating a path entirely from scratch, 
we can start with a pre-built or partially constructed path, which already provides a solid foundation. 
Additionally, reducing the number of waypoints in the path can streamline the computation, allowing the 
algorithm to focus on refining an already good solution rather than calculating every detail from the 
ground up. This approach maintains the strength of meta-heuristics while significantly speeding up the 
planning process, making it more practical for real-time or near-real-time applications.
...
inside the stations where less obstacles and dynamics are expected. In addition the geometric solution is scalable to 
any station whose properties are explicited. This makes the overall solution and the autonomous forklifts simple to
...
summarize with psudeo code or flowchart:
    How to reach the pallet? Pre-defined path: pattern based path
    Where to transition? Polygones
    How to optimize transition path? meta-heuristic
    How to create the path? interpolate waypoints using splines
    Obstacles? splines + meta-heuristic
