...
(PosYsub\_polygon). The algorithm concludes by returning the two created subpolygons, effectively segmenting the 
...
Path optimization), the waypoints are created. The waypoints depicted on the simulation figure are created relating to the 
Robot, the destination and the transition positions. In a simple case where no obstacles are around, orientation points are 
added in front of the 3 positions in order to guide the robot during the first acceleration or deceleration by driving 
in a straightforward or straightbackward motion depending on the driving direction. 
